 ...
 */
class TopDealsController extends ControllerBase {
...
//            setcookie('storeid', $storeId,time() + (86400 * 30), "/"); // 86400 = 1 day            
  ...
  function getAllTopDeals($store_id,$mall_id) {
       $data = $this->getStoreData($mall_id,$filterByCategory); 
...
//        }
          global $base_url;
         $storeUrl = $base_url.'/mall/'.$store_id.'/'.$mall_id.'/all';
...
//  }
    ...
    static public function getStoreData($mallId,$filterByCategory=''){        
        //$data = [];
        if(empty($mallId)){return ; }
        
        //echo $filterByCategory;die;
        $search = $_REQUEST['search'];
        if(!empty($search) && $search!=='all'){
            
            $term = \Drupal::entityTypeManager()->getStorage('taxonomy_term')
                    ->loadByProperties(['name' => $filterByCategory, 'vid' => 'category']);
            $term = reset($term);
            $term_id = $term->id();
            $nodes = \Drupal::entityTypeManager()
        ->getStorage('node')
        ->loadByProperties(['type' => 'top_deal', 'field_mall_id' => $mallId,'field_catogery' => $term_id,'status'=>1,'field_is_feature'=>1]);
        
        }else{
            
            $nodes = \Drupal::entityTypeManager()
        ->getStorage('node')
        ->loadByProperties(['type' => 'top_deal', 'field_mall_id' => $mallId,'status'=>1,'field_is_feature'=>1]);
    ...
    if(!empty($nodes)){
        ...
        if (!empty($node)) {       
            ...
            $brandLogoId = $node->field_brand_logo->target_id;
            if(!empty($brandLogoId)){
                $logofile = \Drupal\file\Entity\File::load($brandLogoId);
                $logoPath = $logofile->getFileUri();
                $brandLogoUrl = \Drupal\Core\Url::fromUri(file_create_url($logoPath), ['https' => 
				!empty(getenv('APP_ENV')) ? true : false
		       	])->toString();
            }

            $featuresImagesId = $node->field_features_image->target_id;                      
            if(!empty($featuresImagesId)){
                $file = \Drupal\file\Entity\File::load($featuresImagesId);
                $path = $file->getFileUri();
                $featuresImageImageurl = \Drupal\Core\Url::fromUri(file_create_url($path), ['https' => 
				!empty(getenv('APP_ENV')) ? true : false
			     ])->toString();
            }
          //  dump($node);die;
            $_SESSION['lifafa_store']['store_terminal_id'] = $node->id();;
            $categoryId = $node->field_catogery->target_id;
            if(!empty($categoryId)){
                $term = \Drupal::entityTypeManager()->getStorage('taxonomy_term')->load($categoryId);
                $categoryTitle = $term->name->value;
            }
            $typesId = $node->field_types->target_id;
            if(!empty($typesId)){
                $term = \Drupal::entityTypeManager()->getStorage('taxonomy_term')->load($typesId);
                $types = $term->name->value;
            }
            
            $data[$nodeId]['title'] = $node->title->value;
            $data[$nodeId]['store_id'] = $node->field_store_id->value;        
            $data[$nodeId]['mall_id'] = $node->field_mall_id->value;        
            
            $data[$nodeId]['brandLogo'] = $brandLogoUrl;
            $data[$nodeId]['featuresImage'] = $featuresImageImageurl;
            $data[$nodeId]['category'] = ($categoryTitle)?$categoryTitle:'';
            $data[$nodeId]['types'] = ($types)?$types:'';
