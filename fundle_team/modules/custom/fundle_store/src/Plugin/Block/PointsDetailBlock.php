 ...
 *
 * @Block(
 *  id = "points_detail_block",
 *  admin_label = @Translation("Points Detail block"),
 * )
 */
class PointsDetailBlock extends BlockBase {
  ...
  public function build() {
       ...
       $landing_page = ['park'=>'ye-bg','eat'=>'rd-bg','book'=>'bl-gr','shop'=>'bl-bg'];
        
        $userWalletPoints = (isset($_REQUEST['userWalletPoints']) && $_REQUEST['userWalletPoints']>0)?$_REQUEST['userWalletPoints']:0;        
        ...
        $point_color = '';
        $point_star_image = 'points';
            if(array_key_exists($first_argument, $landing_page)){                
                $point_color = $landing_page[$first_argument];
                $point_star_image = 'points-'.$landing_page[$first_argument];
            }                
        }                    
        $data['points'] =$userWalletPoints;// $this->getTermDataData($first_argument);
        $data['point_color'] = $point_color;
        $data['star_image'] = $point_star_image;        
        
    $build = [];
    $build['#theme'] = 'points_detail_block';
