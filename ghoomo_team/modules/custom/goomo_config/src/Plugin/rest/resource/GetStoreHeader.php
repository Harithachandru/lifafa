...
class GetStoreHeader extends ResourceBase
{
    ...
    public function get($storeId)
    {
        ...
        $isRNView = $request->query->get('isRNView');
        $device = $request->query->get('device');
        $isFrom = $request->query->get('isFrom');
        $isRNView = ($isRNView == 'true') ? 1 : 0;
        if ($islogged == 'true') {
            $data['store_header']  = getLoggedStoreHeader($storeId, $isRNView, $device, $isFrom);
        } else {
            $data['store_header']  = getStoreHeader($storeId);
        }
        $data['store_fevicon']  = getfevicon($storeId);
